...
it('should be able to find a user by email', function () {
	...
	$foundUsers = $repository->whereEmail($usersEmails)->all();
...
it('should be able to find a user by id', function () {
	...
	$foundUsers = $repository->whereId($usersIds)->all();
	$foundUsersIds = $foundUsers->map(fn ($user) => $user->id)->toArray();

	sort($usersIds);
	sort($foundUsersIds);

	expect(count($foundUsers))->toBe(5);
	expect($foundUsersIds)->toBe($usersIds);
});

it('should be able to search for a user by email', function () {
	$users = Doctrine::factory(User::class, 10);

	$repository = app()->get(UserRepositoryInterface::class);
	$foundUsers = $repository->search('email.com')->all();

	$usersEmails = array_map(fn ($user) => $user->email, $users);
	$usersEmails = array_filter($usersEmails, fn ($email) => strpos($email, 'email.com') !== false);
